...
const ALT_LABEL = /^(남은자리\s*\d{1,3}\s*[명석자리]*|예약완료|예약마감|마감|만석|매진|예약하기|대기하기|개인사정|휴항|결항|출조취소|입금자|입금대기|예약자|예약대기|대기자|취소자)$/;
...
// 입금자·입금대기·예약완료 명단만 셉니다. 대기자·예약대기·취소자는 자리를 차지하지 않습니다.
//
// 어청도피싱호는 라벨을 "입금자/대기자"가 아니라 **"예약완료/예약대기"**로 적습니다.
// 그런데 `예약대기`가 ALT_LABEL에도 SKIP_LINE에도 없어서 그 이미지가 통째로 지워졌고,
// 경계가 사라진 대기자 명단이 찬 자리로 같이 세어졌습니다 — 9/19이 예약완료 19명 ·
// 예약대기 5명 · 남은자리 1명(정원 20)인데 정원이 25명으로 나왔습니다(1 + 19 + 5).
// 예약대기가 자리를 차지하지 않는다는 것은 그 예약판의 셈이 말해줍니다(19 + 1 = 20).
// `예약완료`도 TAKEN_LINE에 넣습니다 — 안 넣으면 대기자 줄이 먼저 오는 예약판에서
// taking이 false로 굳어 정작 찬 자리를 못 셉니다.
const TAKEN_LINE = /(입금|예약확정|확정|예약완료|^예약자)/;
const SKIP_LINE = /(예약대기|대기자|취소|환불)/;
...
function countTakenSeats(text, mode = 'seatNumbers') {
  ...
  for (const part of text.split(/(?=입금대기|입금자|예약확정|예약완료|예약대기|대기자|취소자|환불)/)) {
