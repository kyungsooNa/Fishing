 ...
 *
 * ## 더피싱도 같은 병이었습니다 (2026-09-19)
 *
 * 선상24를 25곳으로 줄이는 동안 더피싱 109곳은 매시간 그대로 돌고 있었습니다. 그쪽이
 * 먼저 무너졌습니다 — `data.json` 12번치를 재보면 **09-19 05:31부터 22:57까지 줄곧
 * 0/109**이고, 그 앞 09-18도 0/109와 107/109를 오갑니다. 파서가 깨진 게 아닙니다:
 * 같은 코드로 09-18 14:49에 107곳이 성공했습니다.
 *
 * **끊기는 자리가 선상24와 같습니다.** 유일한 부분 성공이 09-19 01:25의 **31/109**로,
 * 선상24에서 잰 27~31과 같은 띠입니다. 그래서 여기도 25 — 관측된 하한(31)보다 한 칸
 * 아래입니다. 109곳이면 매시 수집으로 한 바퀴가 **약 5시간**입니다.
 *
 * 막히는 모양은 두 단계로 나옵니다. 먼저 200으로 오되 출조가 0건인 페이지가 오고
 * (`예약 페이지에서 출조를 못 찾았습니다` 109곳), 더 두들기면 연결 자체를 안 받습니다
 * — ports 실행(run 35452539845)에서 더피싱 64곳이 전부 `115.68.85.4`로 시간초과였고,
 * **같은 실행의 선상24·자체 사이트는 3초에 멀쩡히 성공했습니다.** 우리 쪽 네트워크가
 * 아니라 저쪽이 이 IP를 막고 있다는 뜻입니다.
 *
 * `PLATFORM_TIMEOUT_LIMIT`은 이걸 못 막습니다. 그건 **이번 실행**을 중간에 끊을 뿐
