...
// 정비일은 잔여석이 남은 채로 옵니다 — 무창포 대진피싱은 "남은자리 20명"인 배 행 옆에
// "배정비일 / 배 점검으로 예약을 받지 않습니다"를 적어둡니다. 그래서 숫자를 뒤집을 수 있는
// 이 목록에 들어가야 합니다(CLOSED_WORDS에 넣으면 잔여석이 있는 날은 그대로 예약가능입니다).
// "배·선박"이 앞에 붙은 것만 봅니다 — 한글에는 \b가 없어서 "장비점검"·"채비정비"처럼
// 앞 글자가 붙은 말이 그대로 통과합니다(AGENTS.md의 항구 후보와 같은 함정).
const OFF_PATTERNS = [/(?<!꾸)미출조/, /(?:배|선박)(?:정비|점검|수리)/, /(?:정비|점검)중/];
