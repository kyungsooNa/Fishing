...
- **별점(개인 리뷰)** — 선사마다 별 다섯 개로 매깁니다. 같은 별을 다시 누르면 지웁니다. [아래](#별점개인-리뷰)를 보세요.
...
선사마다 별 다섯 개로 매기는 개인 평가입니다. **[시스템 관리](#시스템-관리-페이지) 페이지에서만
매기고**, 현황판에는 선사 이름 옆에 **읽기 전용으로** 보입니다. 표를 훑다가 잘못 눌러 점수가
조용히 바뀌는 일이 없게 매기는 곳을 한 군데로 뒀습니다.

- 별을 누르면 그 점수, **같은 별을 다시 누르면 지웁니다.** 0점과 "안 매김"은 구별하지 않습니다.
- 즐겨찾기와 같이 브라우저에만 저장합니다(`localStorage`의 `fishing:ratings`). **기기마다 따로**
  남고, 시크릿 창이나 사이트 데이터를 지우면 사라집니다. 서버로도, 커밋으로도 나가지 않습니다.
- 그래서 **읽기 전용 모드(GitHub Pages)에서도 매길 수 있습니다.** registry를 고치는 값이 아닙니다.
- 안 매긴 선사에는 아무것도 안 붙입니다. 288곳에 ☆☆☆☆☆를 깔면 표가 안 읽힙니다.
- 같은 배가 여러 사이트에 올라와 합쳐진 줄은 사이트마다 따로 보여줍니다 — 별점은 배가 아니라
  **선사(사이트 id)에** 매기기 때문입니다.
