...
node collect.js                   # 전체
node status.js                    # 지금 무엇을 얼마나 보고 있나 (계열별 커버리지·실패 비율)
...
node status.js --from tmp/monitor.json  # 로컬 수집 결과로
node status.js --json                   # 기계가 읽을 형식 (추이를 기록할 때)
node status.js --sites 선상24            # 그 계열의 사이트 id 전부
node status.js --all                    # 최신이 아닌 곳을 12곳만 말고 전부
```

```
■ 전체
  사이트 290곳 등록 · 267곳 집계 (꺼짐 23)
  출조 10,060건
  마지막 수집: 성공 155 · 실패 0 · 보류 112 · 기록 없음 0
  요청한 곳 성공률 100.0% · 이번에 값을 새로 받은 곳 58.1%

■ 계열별
  계열          사이트  성공  실패  보류  기록없음  성공률    최신   출조
  선상24           154   154     0     0         0  100.0%  100.0%  6,193  (전체의 61.6%)
  더피싱(상세)     111     0     0   111         0       -    0.0%  3,806  (전체의 37.8%)
```

**성공률과 "최신"은 다른 숫자입니다.** 성공률은 실제로 요청한 곳 중 성공한 비율이고,
최신은 집계 대상 전체 중 이번에 값을 새로 받아온 비율입니다. 위 예에서 요청한 곳은
전부 성공해 100%지만, 더피싱 111곳이 timeout 백오프로 보류라 화면의 38%는 몇 시간 전
값입니다. 성공률만 보면 "이상 없음"으로 읽힙니다.

`registry`(보기로 한 것)와 `data.json`(실제로 본 것)을 맞춰 세기 때문에 둘 중 한쪽에만
있는 사이트도 빠지지 않습니다. **모든 분류는 합이 전체와 같습니다** — 계열별 사이트를
더하면 집계 대상이 되고, 계열별 출조를 더하면 `data.json`의 출조 수가 됩니다.
합쳐진 출조는 대표 사이트 계열로 한 번만 세고, 몇 줄이 합쳐졌는지는 따로 알려줍니다.
