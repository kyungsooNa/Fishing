...
- **즐겨찾기** — 배별 칸의 **☆** 로 그 자리에서 담고 뺍니다. 담아둔 배는 아래 목록에서 한눈에 보고 지웁니다. 이건 브라우저에 저장되는 값이라 **읽기 전용 모드(GitHub Pages)에서도 됩니다.**
- **숨기기** — 선사 이름 옆의 **✕** 로 그 선사를, 배별 칸의 **✕** 로 그 배를 내 현황판에서 뺍니다. 숨긴 것은 아래 **숨긴 것** 표에 모입니다.
- **숨긴 것** — 숨긴 선사·배·항구·지역을 한 표로 보고 되돌립니다. 이것도 브라우저에 저장되는 값입니다. [아래](#관심-없는-것-숨기기)를 보세요.
...
지도도 같이 좁혀집니다. [시스템 관리](#시스템-관리-페이지) 표의 **배별 즐겨찾기·별점** 칸에서도
담습니다 — 선사 목록을 훑다가 담고 싶을 때 현황판으로 건너가 그 배를 다시 찾지 않아도 됩니다.
...
- 관리 화면의 ☆ 는 **수집이 읽은 배 이름·항구**로 담습니다. 그래서 수집된 출조가 없는 배에는
  ☆ 가 안 붙습니다 — 담을 수는 있는데 현황판에서 영영 안 보이는 것이 가장 나쁩니다.
...
배마다 별 다섯 개로 매기는 개인 평가입니다. 현황판 표의 선사 칸과
[시스템 관리](#시스템-관리-페이지) 페이지 양쪽에서 매깁니다. 표를 훑다가 잘못 눌러 점수가 조용히
바뀌는 일이 없게 **두 번 눌러야 바뀝니다** — 한 번 누르면 고르는 칸이 열릴 뿐이고, 거기서 점수를
골라야 저장됩니다.
...
안 갈 선사·배·항구·지역을 **내 화면에서만** 안 보이게 합니다. 전국을 다 모으면 표가 수천 줄이라,
평생 갈 일 없는 곳이 계속 눈앞을 스쳐 지나갑니다.

- **선사**는 두 군데서 숨깁니다. [시스템 관리](#시스템-관리-페이지) 표에서는 선사 이름 옆의
  **✕**, 현황판에서는 위쪽 **전체 선사** 필터 메뉴의 **✕** 입니다. 관리 화면 표가 곧 선사
  목록이라, 안 갈 곳을 훑다가 그 자리에서 뺄 수 있습니다. 숨긴 선사는 그 줄이 흐려지고 ✕ 가
  **↩** 로 바뀝니다.
- **항구·지역**은 위쪽 필터 메뉴(전체 항구 / 전체 지역)를 펼치면 항목마다 붙어 있는 **✕** 로
  숨깁니다. 체크(보기)와는 다른 버튼이라 ✕ 를 눌러도 체크가 켜지지 않습니다.
- **배**는 두 군데서 숨깁니다. 현황판에서는 표의 별점 칸을 눌러 열리는 칸의 **숨기기**,
  [시스템 관리](#시스템-관리-페이지) 표에서는 배 이름 옆의 **✕** 입니다. 관리 화면은 선사마다 배
  이름이 이미 펼쳐져 있어, 안 갈 배를 훑다가 그 자리에서 바로 뺄 수 있습니다. 숨긴 배는 그 줄이
  흐려지고 ✕ 가 **↩** 로 바뀌어, 같은 자리에서 바로 되돌립니다.
- 숨긴 것이 하나라도 있으면 화면 위쪽에 **`숨긴 것 N개`** 패널이 열립니다. 거기서 하나씩
  **다시 보기**, 또는 맨 아래 **전부 다시 보기** 로 되돌립니다. **푸는 길이 거는 길보다 먼저입니다** —
  못 풀면 "배가 안 뜬다"로 보이고 그건 고장과 구별이 안 됩니다.
- 되돌리는 자리는 두 군데입니다. 현황판 패널은 숨긴 것이 있을 때만 열려서, 무엇을 숨겼는지
  훑어보려면 [시스템 관리](#시스템-관리-페이지)의 **숨긴 것** 표가 낫습니다. 이름이 바뀌어 숨김이
  더는 안 먹는 값도 거기서 **지금 목록에 없음** 으로 뜹니다.

### 수집을 끄는 것과 다릅니다

[시스템 관리](#시스템-관리-페이지)의 켜고 끄기(`registry.json`의 `enabled`)는 **모두에게** 영향을 주고
그 선사 자리를 아예 못 잡게 만듭니다. 숨기기는 즐겨찾기·별점과 같은 급의 **개인 기록**이라
(`localStorage`의 `fishing:muted`) 서버로 안 가고 이 브라우저에만 남습니다. 그래서
**읽기 전용 모드(GitHub Pages)에서도 됩니다.**

- **감시와 알림은 그대로입니다.** 숨기기는 표를 그릴 때만 씁니다. 숨긴 배라도 걸어둔 감시는
  계속 돌고 알림도 옵니다.
- **합쳐진 줄은 출처가 모두 숨긴 선사일 때만 숨습니다.** 한 곳이라도 안 숨긴 사이트에서 올라온
  자리는 그대로 보입니다 — 이 화면은 취소석을 잡으려고 보는 화면이라, 숨기기가 자리를 놓치는
  쪽으로 기울면 안 됩니다.
- 배는 즐겨찾기와 **같은 키(배 이름 + 출항지)** 입니다. 오전·오후로 나뉜 배는 한 번에 숨고,
  다른 지역의 같은 이름 배는 안 숨습니다.
- 선사는 id가 아니라 **이름으로** 숨깁니다. 이름 표기가 바뀌면 숨김이 풀려 **다시 보이는 쪽으로**
  틀립니다 — 조용히 사라지는 것보다 낫습니다.
- 기기마다 따로 남고, 시크릿 창이나 사이트 데이터를 지우면 사라집니다. 저장이 막힌 브라우저에서는
  숨기지 않고 그 자리에 막힌 이유를 적습니다.
