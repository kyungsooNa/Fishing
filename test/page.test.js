...
test('가까운 일정과 90일 장기 일정을 나눠 읽는다', () => {
  assert.match(inline, /fetch\('future\.json\?t=' \+ Date\.now\(\)/);
  assert.match(inline, /Date\.now\(\) - FUTURE_READ_AT < 60 \* 60 \* 1000/,
    '큰 장기 파일은 30초 갱신 때마다 다시 받지 않습니다');
  assert.match(inline, /trips: \[\.\.\.\(near\.trips \?\? \[\]\), \.\.\.\(future\.trips \?\? \[\]\)\]/,
    '화면에서는 가까운 일정과 장기 일정을 합쳐야 합니다');
});
