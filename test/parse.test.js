...
// 선상24 예약판은 **모든 행에** 상태 범례를 같이 적습니다 — "예약완료 입금대기 취소대기
// 취소완료 예약대기 출조대기 점검일". 그래서 "점검"을 낱말 단위로 거르지 않고 그냥 찾으면
// 그 판의 거의 모든 행이 휴항이 됩니다(2026-09-14 수집본에서 "점검" 글자가 든 행이 129건이고
// 그중 127건이 이 범례였습니다). 정비·점검은 "배·선박"이 앞에 붙은 것만 봅니다.
test('선상24 예약판 범례의 "점검일"은 휴항이 아니다', () => {
  const legend = '태헌호 어종 : 주꾸미 / 선상 운항시간 : 05:00 ~ 15:00 ' +
    '예약완료 입금대기 취소대기 취소완료 예약대기 출조대기 점검일';

  assert.equal(toStatus(legend, null), STATUS.CLOSED, '범례의 점검일로 그 배가 휴항이 되면 안 됩니다');
  assert.equal(toStatus(legend.replace('예약완료', '남은자리 5명'), 5), STATUS.OPEN,
    '자리가 있는 행이 범례 때문에 사라지면 잡을 수 있는 자리를 놓칩니다');
});

// 정우레져호는 이 문구를 배 행에 직접 적습니다(취소자 행이 아닙니다). 예약판에 잔여 숫자가
// 없어서 고치기 전에는 "확인필요"로 떴습니다 — TODO의 "잔여" 항목이 짚어둔 그 한 건입니다.
test('배 행에 직접 적힌 "배 점검으로 예약을 받지 않습니다"도 휴항이다', () => {
  const text = '2026년 09월 14일, 월요일, 10물 정우레져호 배 점검으로 예약을 받지 않습니다. 쭈꾸미 입금자 한*희님';

  assert.equal(toStatus(text, null), STATUS.OFF);
});
