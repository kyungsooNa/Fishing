...
// 선상24 선사들이 실제로 쓰는 말은 "출조항구"입니다. 라벨 목록에 없어서 8곳이 값 대신
// 후보로만 나왔고, 후보는 채울 수 없으니 그대로 빈 칸으로 남아 있었습니다.
test('출조항구 라벨을 값으로 읽는다', () => {
  assert.equal(pickPort('※ 출조항구 : 물개항').value, '물개항');
  assert.equal(pickPort('※ 출조항구: 신촌항').value, '신촌항');
  assert.equal(pickPort('출조항 : 홍원항').value, '홍원항');
});

// "출조점"은 출조항구가 아니라 승선명부를 쓰는 가게 주소입니다. 항 이름이 아닌 것을
// 라벨로 읽으면 그대로 신원이 되어 엉뚱한 배가 합쳐집니다.
test('출조점 주소는 출조항구로 읽지 않는다', () => {
  assert.deepEqual(pickPort('출조점 : 충남 서천군 서면 홍원길 101-1'), { value: null, candidates: [] });
});
