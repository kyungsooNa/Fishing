...
const PORTS = {
  ...
  '인천 옹진 진두항': { lat: 37.24, lng: 126.48 },   // 여수 돌산에도 진두항이 있지만 여기엔 없습니다
};
...
// PR #281에서 나왔습니다. 양파호(bikini) 페이지에는 대천항과 회변항이 같이 적혀
// 있는데, 회변항이 ports.json에 없다는 이유로 대천항이 혼자 남아 채워졌습니다 —
// 근거로 고른 게 아니라 목록에 없어서 이긴 값입니다. 라벨 경로는 이미 막고 있었는데
// (장곰포항·구매항) 라벨 없는 "모임" 경로에는 같은 관문이 없었습니다.
test('모르는 항구가 같이 적혀 있으면 아는 쪽이 혼자 남았다고 고르지 않는다', () => {
  const both = pickPort('오시는길 : 충남 홍성군 서부면 남당항로 213 남당항 또는 회변항에서 뜹니다', PORTS);
  assert.equal(both.value, null, '회변항이 어디 것인지 몰라 남당항이 이긴 셈입니다');
  assert.ok(both.candidates.includes('회변항'), '사람이 고를 수 있게 후보로는 남깁니다');

  // 지나가는 말로 적힌 **다른 지역** 항구는 이 관문에 안 걸립니다 — 목록에 있어서
  // 시·군이 다르다는 것까지 아는 이름이라, 근거가 아니라고 보여줄 수 있습니다.
  const passing = pickPort('오시는길 : 충남 홍성군 서부면 남당항로 213 남당항에서 뜨고 오천항 앞바다로 갑니다', PORTS);
  assert.equal(passing.value, '충남 홍성 남당항');

  // 항구의 갈래를 적어둔 것도 안 걸립니다 — 이름이 아니라 등급입니다.
  const graded = pickPort('오시는길 : 충남 홍성군 서부면 남당항로 213 국가어항 남당항입니다', PORTS);
  assert.equal(graded.value, '충남 홍성 남당항');
});

// 라벨이 붙은 값은 이 관문을 안 지납니다. 라벨은 그 자체가 근거라, 페이지가 다른 항을
// 지나가는 말로 적었다고 값을 버릴 이유가 없습니다(그 경우는 위의 장곰포항 시험이 봅니다).
test('라벨로 찾은 값은 모르는 이름이 같이 있어도 그대로 값이다', () => {
  const labeled = pickPort('오시는길 : 충남 홍성군 서부면 남당항로 213 회변항 소식 출항지 : 남당항', PORTS);
  assert.equal(labeled.value, '충남 홍성 남당항');
});

// 자동 실행이 채운 값을 보다 나왔습니다. 스텔론1호·베테랑피싱은 주소가 전남 여수시
// 돌산읍인데 "인천 옹진 진두항"이 들어갔습니다 — 여수 돌산에도 진두항이 있지만
// ports.json에는 인천 것 하나뿐이라, 이름이 하나뿐이라는 이유로 골랐습니다.
// 여수 배가 인천에 핀이 찍히고 인천 배와 한 줄로 합쳐집니다(core/merge.js).
test('주소의 시·군에 그 이름의 항구가 없으면 다른 데 항구다', () => {
  const yeosu = pickPort('오시는길 : 전남 여수시 돌산읍 진두해안길 68 출항지 : 진두항', PORTS);
  assert.equal(yeosu.value, null, '같은 이름의 다른 지역 항구를 집으면 안 됩니다');
  assert.ok(yeosu.candidates.includes('진두항'), '사람이 고를 수 있게 후보로는 남깁니다');

  // 주소가 그 항구의 시·군이면 그대로 값입니다.
  const incheon = pickPort('오시는길 : 인천시 옹진군 영흥면 영흥로 109-10 출항지 : 진두항', PORTS);
